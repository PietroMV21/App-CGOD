...
data_inicio_contagem = datetime.date(2026, 8, 1) # Referência para a barra de progresso
...
def janela_adicionar_post():
    ...
    c = st.selectbox("Categoria", list(CATEGORIAS_INFO.keys()))
    if st.button("Salvar", type="primary"):
        novo = {"id": int(datetime.datetime.now().timestamp()), "titulo": t, "responsavel": r, "data": d.strftime("%Y-%m-%d"), "categoria": c, "status": "Programado"}
...
if tela == "📊 Dashboard Geral":
    ...
    df = pd.DataFrame(st.session_state.posts) if total > 0 else pd.DataFrame(columns=['status', 'data', 'categoria'])
    
    # Atualização de status automática para lógica de atraso
    if not df.empty:
        df['data_dt'] = pd.to_datetime(df['data']).dt.date
        df.loc[(df['data_dt'] < data_hoje) & (df['status'] == 'Programado'), 'status'] = 'Atrasado'

    # Linha 1: Métricas e Progresso
    ...
    with col_m1:
        ...
        total_dias = (data_evento - data_inicio_contagem).days
        dias_passados = (data_hoje - data_inicio_contagem).days
        progresso = min(max(dias_passados / total_dias, 0.0), 1.0)
        st.progress(progresso, text=f"Progresso até o evento: {int(progresso*100)}%")

    with col_m2:
        if not df.empty:
            fig = px.pie(df, names='status', title="Status das Postagens", 
                         color='status', color_discrete_map={'Concluído': '#2ecc71', 'Programado': '#3498db', 'Atrasado': '#e74c3c'},
                         hole=0.4)
    ...
    # Linha 2: Próximas Postagens (Blocos Horizontais)
            ...
            with col_list[idx]:
                st.markdown(f"""
                    <div style="background-color: {cor}; padding: 15px; border-radius: 10px; color: white; min-height: 120px; box-shadow: 2px 2px 5px rgba(0,0,0,0.1)">
                        ...
                        <small>Ref: {row['responsavel']}</small>
                    </div>
            ...
            with cols[i]:
                ...
                for p in posts_dia:
                    ...
                    if st.button(p['titulo'], key=f"cal_{p['id']}", use_container_width=True):
                        ...
                        st.session_state.edit_mode = False
                        st.rerun()
                    # CSS para colorir o botão após renderizar
                    st.markdown(f"<style>div[data-testid='stButton'] button[key='cal_{p['id']}'] {{ background-color: {cor} !important; color: white !important; border: none; font-size: 11px; padding: 2px; }}</style>", unsafe_allow_html=True)

    # Painel de Detalhes (Âncora de Rolagem)
    if st.session_state.post_selecionado_id:
        st.markdown("<div id='detalhes'></div>", unsafe_allow_html=True)
        st.divider()
        ...
        if post_idx is not None:
            ...
            with st.container(border=True):
                ...
                if st.session_state.edit_mode:
                    ...
                    p['categoria'] = col_e2.selectbox("Categoria", list(CATEGORIAS_INFO.keys()), index=list(CATEGORIAS_INFO.keys()).index(p['categoria']))
                    ...
                    col_v2.markdown(f"**Categoria:** {p['categoria']}")
                    col_v2.markdown(f"**Status:** `{p['status']}`")
